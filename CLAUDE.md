...
> - **一键安装** — 通过 [`vercel-labs/skills`](https://github.com/vercel-labs/skills) 安装器，支持 **54 个 agent harness**（Claude Code / OpenClaw / Hermes-Agent / Cursor / Codex / OpenCode / Windsurf 等）。本仓库为 `Carlos-Zen/asknow`（fork 用户把命令里的 `Carlos-Zen` 换成自己的 GitHub 用户名）：
>   ```bash
...
>   ```
> - `npx skills` 只拷 SKILL.md，不跑 bootstrap。`asknow` skill 首次激活时**自动**判断并按需在项目根 `./` 下创建知识库骨架；或预先 `curl -fsSL https://raw.githubusercontent.com/Carlos-Zen/asknow/main/scripts/bootstrap.sh | bash` 手动 bootstrap。
> - 完全本地手动：克隆仓库 → `bash scripts/bootstrap.sh` → 把 `.claude/skills/` 复制到目标项目。

## 目录结构

```
./                        # 知识库根 = 项目根（与 bootstrap.sh 和 asknow skill 的 data-root 一致）
