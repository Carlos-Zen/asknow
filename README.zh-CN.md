...
**Knowledge Base Builder = Claude Code + 1 个 orchestrator skill + 3 个执行 skill**（可选加一段 `CLAUDE.md` snippet 启用始终在线模式）。
...
├── FEISHU_MAP.md         # 飞书同步映射（启用 feishu-sync 时）
...
每篇笔记都是一份带 YAML frontmatter 的独立 Markdown：
...
| 沉淀深度 | 取决于你当时心情 | 0 | **`knowledge-write` skill 强制四维度思考（至少 3 类）+ 最低字数** |
...
**关键一步：第二天再来。** 你问："基于我已经调研过的 Harvey 和 Cursor，结合我对应用层格局的理解，帮我设计一个 agentic commerce 的冷启动方案。"——Claude 直接读取这些笔记把它们当作上下文，给出的方案是基于你过往研究的延伸，而不是从零开始的通用建议。这就是这套机制正在做的事。

---

## 内置指令（全部用自然语言）

直接对 Claude 说就行，**不用记 slash 命令**。所有指令通过 skill description 关键词匹配触发。

### 写入控制

| 用户说 | 作用 |
|--------|------|
| "这次不要记" / "跳过沉淀" / "别记到知识库" | 本回合不写入 |
| "归到 <目录>" / "放到 decisions" | 强制归入指定目录 |
| "用 <X> 作标签" / "打标签为 X" | 手动指定标签 |
| "更新 <文件>" / "改进已有的 Y 文件" | 强制更新指定文件而不新建 |
| "标重要" / "高优先级" | 标记为高优先级（写入 frontmatter） |

### 同步控制

| 用户说 | 作用 |
|--------|------|
| "本次不要同步" / "跳过同步" / "这次先别同步" | 本回合跳过自动同步 |
| "关闭自动同步" / "暂停自动同步" | 持久化关闭自动同步开关 |
| "开启自动同步" / "恢复自动同步" | 持久化开启 |
| "切换同步目标" / "换个空间" / "同步到 <URL>" | 切换飞书同步目标（贴 URL 自动解析；说"列出可选"则列空间） |
| "现在同步一下" / "立即同步" / "推到飞书" | 立即触发一次完整同步 |

### 维护

| 用户说 | 作用 |
|--------|------|
| "帮我维护下知识库" / "整理知识库" / "清理重复" | 调用 `knowledge-maintenance` skill 全库扫描修复 |
...
**做什么：** 总控每次交互的 4 步流程（检索 → 回答 → 沉淀 → 收尾自动同步），决定何时委派给下面 3 个执行 skill，识别用户的自然语言控制指令（"跳过沉淀""跳过同步""切换同步目标"等，见"内置指令"段）。
...
**强约束：** `knowledge-write` skill 规定了**深度与篇幅基线**——`concepts` 类笔记 800–2000 字、`decisions` 必须包含 trade-off 表格、`howto` 每步必须解释原理、`cases` 必须含"现象 / 过程 / 根因 / 修复 / 启示"五段。Claude 写不够这个深度时，会优先选择**追加进已有文件**而非新建一篇浅薄笔记。这从根本上避免了知识库变成"流水账合集"。
...
**何时触发：**
- 用户说"同步到飞书""推送到 wiki""推送到飞书文档""立即同步" → 完整同步
- 用户说"切换同步目标""换个空间""换个文件夹" → 重新初始化
...
完整指令列表见上面"内置指令"段（全部用自然语言表达，不需要 slash 前缀）。
...
A：`asknow` / `knowledge-write` skill 里有明确的分类规则、文件粒度规则、深度基线、查重逻辑。`knowledge-maintenance` skill 会定期清理。最坏情况你也只是多了几个文件，删除即可——所有内容都是纯 Markdown。

**Q：会不会一聊天就疯狂建文件？**
A：不会。`asknow` skill 明确规定**闲聊、纯指令、琐碎内容不写入**；多轮对话默认追加进同一文件而非新建；写不够深度基线时优先合并进已有文件。

**Q：可以用别的 AI 吗（GPT、Gemini）？**
A：技术上 SKILL.md 里的规则可以迁移给其他 AI，但 Skill 系统是 Claude Code 特有的。其他 AI 上效果会打折。
